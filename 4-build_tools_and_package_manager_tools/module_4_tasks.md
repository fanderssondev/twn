**05 - Checklist Build Tools & Package Manager**
By TechWorld with Nana

---
## Video Overview
- ★ [01 - Introduction to Build & Package Manager Tools](#01-introduction-to-build--package-manager-tools)
- ★ [02 - Install Build Tools](#02-install-build-tools)
- ★ [03 - Windows Installation Help - Part 1 & 2](#03-windows-installation-help---part-1--2)
- ★ [04 - MacOS/Unix Installation Help](#04-macosunix-installation-help)
- ★ [05 - Build Artifact](#05-build-artifact)
- ★ [06 - Build Tools for Development](#06-build-tools-for-development)
- ★ [07 - Run the application](#07-run-the-application)
- ★ [08 - Build JavaScript applications](#08-build-javascript-applications)
- ★ [09 - Other Programming Languages](#09-other-programming-languages)
- ★ [10 - Publish artifact](#10-publish-artifact)
- ★ [11 - Build Tools & Docker](#11-build-tools--docker)
- ★ [12 - Build Tools & DevOps](#12-build-tools--devops)

---
<a id="01-introduction-to-build--package-manager-tools"></a>
## 01 - Introduction to Build & Package Manager Tools
- [ ] Watched video

---
<a id="02-install-build-tools"></a>
## 02 - Install Build Tools
- [ ] Watched video
- [ ] Go to Windows or MacOS/Unix Installation Help

### Useful Links:
- Maven: https://maven.apache.org/install.html
- Gradle: https://gradle.org/install/
- Node.js: https://nodejs.org/en/download/ - npm is distributed with Node.js

---
<a id="03-windows-installation-help---part-1--2"></a>
## 03 - Windows Installation Help - Part 1 & 2
- [ ] Watched video
- [ ] Installed everything

### Useful Links:
- Java on Windows: https://docs.oracle.com/en/java/javase/17/install/installation-jdk-microsoft-windows-platforms.html#GUID-0DB9580B-1ACA-4C13-8A83-9780BEDF30BB

---
<a id="04-macosunix-installation-help"></a>
## 04 - MacOS/Unix Installation Help
- [ ] Watched video
- [ ] Installed everything

### Useful Links:
- Homebrew Package Manager: https://brew.sh/
- Java on Mac/Linux: Install using your OS package manager (e.g. brew)

---
<a id="05-build-artifact"></a>
## 05 - Build Artifact
- [ ] Watched video
- [ ] Demo executed
- [ ] Built Java Gradle Project
- [ ] Built Java Maven Project

---
<a id="06-build-tools-for-development"></a>
## 06 - Build Tools for Development
- [ ] Watched video
- [ ] Demo executed - add a new dependency to project

---
<a id="07-run-the-application"></a>
## 07 - Run the application
- [ ] Watched video
- [ ] Demo executed

---
<a id="08-build-javascript-applications"></a>
## 08 - Build JavaScript applications
- [ ] Watched video
- [ ] Demo executed

---
<a id="09-other-programming-languages"></a>
## 09 - Other Programming Languages
- [ ] Watched video

---
<a id="10-publish-artifact"></a>
## 10 - Publish artifact
- [ ] Watched video

---
<a id="11-build-tools--docker"></a>
## 11 - Build Tools & Docker
- [ ] Watched video

---
<a id="12-build-tools--devops"></a>
## 12 - Build Tools & DevOps
- [ ] Watched video
